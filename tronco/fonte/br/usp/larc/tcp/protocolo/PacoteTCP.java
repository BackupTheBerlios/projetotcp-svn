...
/*
 * @(#)PacoteTCP.java 1.0 31/04/2004 Copyleft (L) 2004 Laboratório de
 ...
 * especificação da Apostila II da experiência. Para entender mais detalhes
 * sobre o processo de conversão de campos para se ajustar a especificação do
 * nosso segmento TCP adaptado, consulte as classes CampoTCP e Decoder.
 * 
...
public class PacoteTCP
{
    ...
    /**
     * Atributo que representa o campo de campo de sequencia do header do
     * pacote.
     */
     ...
     *        Valor do atributo ipSimulado local.
     * @param _ipSimuladoRemoto
     *        Valor do atributo ipSimulado remoto.
     * @param _portaLocal
     *        Valor do atributo porta local.
     * @param _portaRemota
     *        Valor do atributo porta remota.
     * @param _numSequencia
     *        Valor do atributo campo numSequencia.
     * @param _numAck
     *        Valor do atributo campo numAck.
     * @param _offset
     *        Valor do atributo offset.
     * @param _controle
     *        Valor do atributo controle.
     * @param _janela
     *        Valor do atributo janela.
     * @param _checksum
     *        Valor do atributo checksum.
     * @param _ponteiroUrgente
     *        Valor do atributo ponteiroUrgente.
     * @param _opcoes
     *        Valor do atributo opcoes.
     * @param _dados
     *        Valor do atributo Parameter.
     */
     ...
     * 
     * @param _texto
     *        Texto codificado em bytes, após ser decodificado altera os
     *        atributos do pacote.
     */
    public PacoteTCP (String _texto)
    {
        if (_texto.length () >= 36)
        {
            ...
            this.ipSimuladoRemoto = new String (this.decodificarIp (_texto.substring (0,
                    6)));
            this.ipSimuladoLocal = new String (this.decodificarIp (_texto.substring (6,
                    12)));
     ...
     * 
     * @param _checksum
     *        O novo valor para o atributo checksum.
     */
    public void setChecksum (int _checksum)
    {
        try
        {
            this.checksum.setValue (_checksum);
        }
        catch (Exception e)
        {
            e.printStackTrace ();
     ...
     * 
     * @param _controle
     *        O novo valor para o atributo controle.
     */
    public void setControle (short _controle)
    {
        try
        {
            this.controle.setValue (_controle);
        }
        catch (Exception e)
        {
            e.printStackTrace ();
     ...
     * 
     * @param _dados
     *        O novo valor para o atributo dados.
     */
     ...
     * 
     * @param _ipSimuladoLocal
     *        O novo valor para o atributo ipSimuladoLocal.
     */
     ...
     * 
     * @param _ipSimuladoRemoto
     *        O novo valor para o atributo ipSimuladoRemoto.
     */
     ...
     * 
     * @param _janela
     *        O novo valor para o atributo janela.
     */
    public void setJanela (int _janela)
    {
        try
        {
            this.janela.setValue (_janela);
        }
        catch (Exception e)
        {
            e.printStackTrace ();
     ...
     * 
     * @param _numAck
     *        O novo valor para o atributo numAck.
     */
    public void setNumAck (long _numAck)
    {
        try
        {
            this.numAck.setValue (_numAck);
        }
        catch (Exception e)
        {
            e.printStackTrace ();
     ...
     * 
     * @param _numSequencia
     *        O novo valor para o atributo numSequencia.
     */
    public void setNumSequencia (long _numSequencia)
    {
        try
        {
            this.numSequencia.setValue (_numSequencia);
        }
        catch (Exception e)
        {
            e.printStackTrace ();
     ...
     * 
     * @param _offset
     *        O novo valor para o atributo offset.
     */
    public void setOffset (short _offset)
    {
        try
        {
            this.offset.setValue (_offset);
        }
        catch (Exception e)
        {
            e.printStackTrace ();
     ...
     * 
     * @param _ponteiroUrgente
     *        O novo valor para o atributo ponteiroUrgente.
     */
    public void setPonteiroUrgente (int _ponteiroUrgente)
    {
        try
        {
            this.ponteiroUrgente.setValue (_ponteiroUrgente);
        }
        catch (Exception e)
        {
            e.printStackTrace ();
     ...
     * 
     * @param _portaLocal
     *        O novo valor para o atributo portaLocal.
     */
    public void setPortaLocal (String _portaLocal)
    {
        try
        {
            this.portaLocal.setValue (Integer.parseInt (_portaLocal));
        }
        catch (Exception e)
        {
            e.printStackTrace ();
     ...
     * 
     * @param _portaRemota
     *        O novo valor para o atributo portaRemota .
     */
    public void setPortaRemota (String _portaRemota)
    {
        try
        {
            this.portaRemota.setValue (Integer.parseInt (_portaRemota));
        }
        catch (Exception e)
        {
            e.printStackTrace ();
    ...
    /**
     * Método que codifica o ipSimulado no tamanho certo das especificações para
     * o header do pacote.
     * 
     * @param _ip
     *        String que contém o ipSimulado a ser codificado.
        ...
        while (str.hasMoreTokens ())
        {
            if (i == 3)
            {
                ...
                ipCodificado = ipCodificado
                               + Decoder.shortToString (Short.parseShort (str2
                                       .nextToken ()));

                //System.out.println("Ic" + i + ": " +
                // Integer.parseInt(ipCodificado));

                ipCodificado = ipCodificado
                               + Decoder.intToString (Integer
                                       .parseInt (str2.nextToken ()));
                //System.out.println("Ic" + i + ": " +
                // Integer.parseInt(ipCodificado));
            }
            else
            {
                ipCodificado = ipCodificado
                               + Decoder.shortToString (Short.parseShort (str
                                       .nextToken ()));
                //                System.out.println("Ic" + i + ": " +
                // Integer.parseInt(ipCodificado));
            }
     ...
     * 
     * @param _ipCodificado
     *        String que contem o ipSimulado codificado.
    ...
    private String decodificarIp (String _ipCodificado)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ipDecodificado = ipDecodificado
                             + Short.toString (Decoder.stringToShort (s.substring (i,
                                     i + 1)));
        ...
        ipDecodificado = ipDecodificado
                         + String.valueOf (Decoder.stringToInt (s.substring (4, 6)));
    ...
    /**
     * Método que ajusta para um novo valor o campo de sequência (1 + (mais) o
     * número de bytes do campo de dados do pacote. No caso do número de
     * sequêcia for maior que 2 elevado a 32 - 1 (igual ao máximo valor do tipo
     * long) ele zera o contador de número de sequência para 0.
     * 
     * @param _num
     *        valor atual do campo de sequencia.
    ...
    public String toString ()
    {
        return this.codificarIp (this.ipSimuladoRemoto)
               + this.codificarIp (this.ipSimuladoLocal) + this.portaRemota.toString ()
               + this.portaLocal.toString () + this.numSequencia.toString ()
               + this.numAck.toString () + this.offset.toString ()
               + this.controle.toString () + this.janela.toString ()
               + this.checksum.toString () + this.ponteiroUrgente.toString ()
               + this.opcoes.toString () + this.getDados ();
    }
    ...
    public static void main (String args[])
    {
        ...
        // ter: 4294967295
        p.setNumSequencia ((long) (Math.pow (2, 32) - 1));
        System.out.println ("Num. Seq.: " + p.getNumSequencia ());
        p.ajustaNumSeq (p.getNumSequencia ());
        System.out.println ("Num. Seq. Incrementado: " + p.getNumSequencia ());
        p.setNumAck (0);
        p.setOffset ((short) 0);
        ...
        System.out.println ("Tamanho Pacote1: " + p.toString ().length ());
