...
/*
 * @(#)TabelaDeConexoes.java 1.0 31/04/2004 Copyleft (L) 2004 Laboratório de
 ...
 * chaveamento de pacotes para as máquinas de estados correspondentes. Note que
 * essa classe estende a classe java.util.Hashtable (ela herda todos os
 * atributos e métodos) e sobrecarrega/sobrepõe alguns métodos da mesma. Estamos
 * usando o conceito de Polimorfismo. Detalhes e dicas de implementação dessa
 * classe podem ser consultadas na API da classe java.util.Hashtable. Procure
 * sempre usar o paradigma Orientado a Objeto, a simplicidade e a criatividade
 * na implementação do seu projeto.
 * 
...
public class TabelaDeConexoes extends Hashtable
{
     ...
     * 
     * @param _conexaoTCP
     *        O objeto conexão que será inserido
     */
     ...
     * 
     * @param _idConexao
     *        O ID da conexão que você quer obter, nulo se não existir a conexão
     *        com o ID passado
     ...
     * 
     * @param _idConexao
     *        o ID da conexão que se quer remover da tabela
     * @throws NullPointerException
     *         Caso não encontre o objeto com o ID passado
     */
     ...
     * Exemplo: //cria um iterador para percorrer um objeto do tipo
     * TabelaDeConexoes Iterator iteratorTabela = (Iterator)
     * tabelaDeConexoes.conexoes(); ConexaoTCP conexao = new Conexao();
     * //percorre todo o iterador while (iteratorTabela.hasNext()) { //imprime
     * todos ID's das conexões que estão na tabela
    ...
    /**
     * Método que retorna um iterador com as chaves dos objetos ConexaoTCP da
     * HashTable
     * 
    ...
    /**
     * Método para visualizar a tabela de conexões Imprime linha por linha o
     * conteúdo da tabela de conexão com a seguinte sintaxe: ID: NumId [
     * IPSimuladoLocal : PortaTCP] [ IPSimuladoRemoto : PortaTCP] Um exemplo:
     * ID: 1 [ 143.107.111.087:4593 : 1024 ] [ 143.107.111.087:4593 : 1025 ] ID:
     * 2 [ 143.107.111.087:4593 : 1026 ] [ 143.107.111.087:4593 : 1027 ]
     * 
    ...
    public String toString ()
    {
        ...
        MessageFormat mf = new MessageFormat ("ID: {0} [ {1} : {2} ] [ {3} : {4} ]\n");
        DecimalFormat df = new DecimalFormat ("0");

        Iterator i = this.conexoes ();

        while (i.hasNext ())
        {
            ConexaoTCP conexao = (ConexaoTCP) i.next ();
            Object[] argumentos = {
                    df.format (conexao.getIdConexao ()),
...
}//fim da classe TabelaDeConexões
